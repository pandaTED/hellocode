...
class Storage:
    ...
    def find_project_by_worktree(self, worktree: str) -> dict | None:
        r = self.conn.execute("SELECT * FROM project WHERE worktree=?", (worktree,)).fetchone()
        return dict(r) if r else None
    ...
    def get_session(self, sid: str) -> dict | None:
        r = self.conn.execute("SELECT * FROM session WHERE id=?", (sid,)).fetchone()
        return dict(r) if r else None
    ...
    def get_message(self, mid: str) -> dict | None:
        r = self.conn.execute("SELECT * FROM message WHERE id=?", (mid,)).fetchone()
        if not r:
            return None
        d = dict(r)
    ...
    def list_parts(self, message_id: str) -> list[dict]:
        ...
        return [{"id": r["id"], **self._r(r["data"])} for r in rows]
    ...
    def list_actors(self, session_id: str, status: str | None = None) -> list[dict]:
        ...
        return [{**dict(r), "tools": self._r(dict(r)["tools"])} for r in rows]
    ...
    def index_memory(self, path: str, scope: str, scope_id: str, mtype: str, body: str, fingerprint: str) -> None:
        self.conn.execute(
            "INSERT OR REPLACE INTO memory_fts_content (rowid, path, scope, scope_id, type, body) "
            "SELECT rowid, ?, ?, ?, ?, ? FROM memory_fts_content WHERE path=?",
            (path, scope, scope_id, mtype, body, path),
        )
        if not self.conn.execute(
            "SELECT 1 FROM memory_fts_content WHERE path=?", (path,)
        ).fetchone():
            self.conn.execute(
                "INSERT INTO memory_fts_content (path, scope, scope_id, type, body) VALUES (?,?,?,?,?)",
    ...
    def search_memory(
        ...
        try:
            ...
            # FTS content table out of sync, rebuild
        ...
        if results:
            ...
            best_score = abs(results[0]["score"])  # best match has largest abs value
            # Keep results with at least 15% of best match quality
    ...
    def _rebuild_memory_fts(self) -> None:
        try:
            ...
            # If rebuild fails, recreate the tables
            try:
    ...
    def search_history(self, query: str, session_id: str | None = None, limit: int = 20) -> list[dict]:
        ...
        if session_id:
            ...
            rows = self.conn.execute(sql, (fts_query, session_id, limit)).fetchall()
        else:
    ...
    def get_events(self, aggregate_id: str, after_seq: int = 0) -> list[dict]:
        ...
        return [{**dict(r), "data": self._r(dict(r)["data"])} for r in rows]
